...
from config import (
    FLASK_SECRET, ADMIN_PASSWORD, DEFAULT_LANGUAGE, ANALYTICS_FILE,
...
from utils.health_report import generate_health_report_pdf
...
_admin_login_attempts: dict[str, deque] = defaultdict(deque)
...
def triage_start():
    ...
    session["db_session_id"]    = db_history.start_session(name, data.get("phone", ""), language)
...
def triage_message():
    ...
    if alert:
        ...
        db_history.log_emergency(
            session.get("db_session_id"), alert.condition, alert.severity, text, alert.action,
        )
        return jsonify({
            "type":           "emergency",
    ...
    prev_question = (
        analyzer.history[-1]["content"]
        if analyzer.history and analyzer.history[-1]["role"] == "assistant"
        else "Initial symptoms"
    )
    result = analyzer.analyze(text)
    session["turn_count"] = session.get("turn_count", 0) + 1
    db_history.log_answer(session.get("db_session_id"), prev_question, text, session["turn_count"])

    if result["complete"]:
        triage = result["triage"] or {}
        session["triage_result"] = triage
        db_history.complete_session(
            session.get("db_session_id"),
            ", ".join(analyzer.patient_state.get("symptoms", [])),
            analyzer.patient_state.get("duration") or "",
            triage,
    ...
    def event_stream():
        while True:
            try:
                ...
                yield "data: {\"error\": \"Clinic not found\"}\n\n"
                return
