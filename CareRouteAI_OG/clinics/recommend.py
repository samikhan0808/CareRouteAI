...
from config import GOOGLE_API_KEY, GEMINI_MODEL, MAX_TOKENS_RANK, CLINIC_RANKING_WEIGHTS
from clinics.clinic_manager import _load_clinics, get_queue
...
def deterministic_score(c: dict, urgency: str, weights: dict | None = None) -> int:
    weights = weights or CLINIC_RANKING_WEIGHTS
    score = weights.get("base", 40)
    score += weights.get("specialty_match", 30) if c["specialty_match"] else 0
    if c["emergency"] and urgency in {"EMERGENCY", "URGENT"}:
        score += weights.get("emergency_bonus", 15)
    elif not c["emergency"] and urgency == "EMERGENCY":
        score -= weights.get("emergency_penalty", 20)
    score += max(0, weights.get("distance_max", 18) - int(c["distance_km"] or 0) * 2)
    score += max(0, weights.get("wait_max", 12) - int(c["est_wait_mins"] or 0) // 5)
    score += min(weights.get("rating_max", 10), int((c["rating"] or 0) * 2))
    return max(0, min(100, score))


def _deterministic_reason(c: dict, triage: dict) -> str:
    return (
        ...
        + (f", predicted condition: {triage.get('predicted_condition')}"
           if triage.get('predicted_condition') else "")
    ).strip(", ")


# ── Deterministic ranking (Gemini does not choose the order) ─────────
def rank_clinics(triage: dict,
    ...
    scored = sorted(
        candidates,
        key=lambda clinic: (-deterministic_score(clinic, urgency), clinic["distance_km"]),
    )[:top_n]
    return {
        "rankings": [
            {
                "clinic_id": clinic["id"],
                "score": deterministic_score(clinic, urgency),
                "lat": clinic["lat"],
                "lng": clinic["lng"],
                "reason": _deterministic_reason(clinic, triage),
                "wait_prediction_mins": clinic["est_wait_mins"],
            }
            for clinic in scored
        ],
        "routing_reason": (
            "Deterministic ranking uses specialty, distance, wait time, "
            "emergency capability, and rating."
        ),
    }
