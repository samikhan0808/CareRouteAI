...
def _fallback_by_specialty(specialty: str, top_n: int = 3,
                            symptoms: list | str | None = None) -> list[dict]:
    """
    ...
    candidates = by_spec.get(str(specialty), []) or by_spec.get("generalPractice", [])

    if symptoms:
        symptom_words = set(_as_text(symptoms).lower().split())
        def relevance(entry):
            kb_symptoms_text = _as_text(entry.get("symptoms", ""))
            kb_words = set(
                (_as_text(kb_symptoms_text) + " " + _as_text(entry.get("condition"))).lower().split()
            )
...
class DiseasePredictor:
    ...
    def predict(self, user_text: str, top_n: int = 3,
        ...
        try:
            ...
            print(f"[DiseasePredictor] KB fallback failed: {fallback_error}")
            return []
