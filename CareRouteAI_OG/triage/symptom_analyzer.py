...
from triage.symptom_state import (
    new_patient_state, update_state, state_to_query_text, build_kb_vocab,
)
...
_KB_VOCAB = build_kb_vocab(_SHARED_RAG.kb)
...
class SymptomAnalyzer:
    def __init__(self, language: str = "en"):
        ...
        self.patient_state = new_patient_state()
    ...
    def analyze(self, user_text: str) -> dict:
        ...
        # Phase 1: accumulate the whole symptom picture, then retrieve local
        # medical knowledge before asking Gemini.
        update_state(self.patient_state, user_text, kb_vocab=_KB_VOCAB)
        rag_query_text = state_to_query_text(self.patient_state, user_text)
        rag_results = self.rag.query(rag_query_text, top_k=3)
        self.last_rag_results = rag_results
        # Use token-aware context trimming before Phase 2 to prevent context
        # window overflow while keeping the most relevant KB guidance.
        ...
        # Phase 2: Gemini interprets the patient's words using the retrieved
        # RAG context. The patient message remains explicit so the model does
        # not mistake KB examples for the patient's actual symptoms.
        augmented_input = user_text + lang_instruction
        if rag_context:
            augmented_input = (
                f"[RELEVANT MEDICAL KNOWLEDGE — internal use only, do not repeat to patient]\n"
                f"{rag_context}\n\n"
        ...
        # If Gemini is unavailable, route from the local RAG evidence instead
        # of repeating a generic error until the turn limit is reached.
        if self.api_unavailable:
            if rag_results or self.turn >= 2:
                forced = self._rule_based_triage(rag_results)
                predictions = self._predict_conditions(triage=forced)
                forced["predictions"] = predictions
                forced["confidence_engine"] = compute_confidence(
                    rag_results, predictions, forced,
                    self.patient_state.get("symptoms", []),
                )
                self.triage_result = forced
                return {
                    "response": self._triage_summary(forced),
                    "complete": True,
                    "triage": forced,
                    "rag_matches": rag_results,
                    "predictions": predictions,
                }
            followups = get_followups("generalPractice", count=1)
            question = followups[0] if followups else (
                "Could you describe where exactly you feel the problem, and since when?"
            )
        ...
        if triage and triage.get("ready"):
            ...
            triage["confidence_engine"] = compute_confidence(
                rag_results, predictions, triage,
                self.patient_state.get("symptoms", []),
            )
            return {
    ...
    def _rule_based_triage(self, rag_results: list[dict]) -> dict:
        """Create a conservative routing result when Gemini is unavailable."""
        top = rag_results[0] if rag_results else {}
        state = self.patient_state
        urgency = "NON-URGENT"
        if state.get("red_flags"):
            urgency = "EMERGENCY"
        elif state.get("severity") in {"severe", "10/10", "9/10", "8/10"}:
            urgency = "URGENT"
        symptoms = state.get("symptoms", [])
        return {
            "ready": True,
            "specialty": top.get("specialty", "generalPractice"),
            "urgency": urgency,
            "confidence": 0.5 if rag_results else 0.35,
            "primaryConcern": top.get("condition", "General health concern"),
            "keySymptoms": symptoms[:8],
            "duration": state.get("duration") or "unknown",
            "summary": "Assessment generated from the medical knowledge base while the AI service is unavailable.",
        }
    ...
    def _call_api(self, user_text: str) -> str:
        ...
        try:
            ...
            # Quota exhaustion and deadlines will not recover within this
            # request. Retrying them only makes the browser/proxy time out.
            first_error = str(first_exc).lower()
            if any(marker in first_error for marker in (
                "429", "quota", "resource exhausted", "deadline", "timeout", "504"
            )):
                self.api_unavailable = True
                return self._service_fallback_message()
            try:
                response = self.chat.send_message(
                    user_text,
                    generation_config=genai.GenerationConfig(
                ...
                self.api_unavailable = True
                return (
                    self._service_fallback_message()
                )

    @staticmethod
    def _service_fallback_message() -> str:
        return (
